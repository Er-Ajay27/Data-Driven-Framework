...
// zohoLoginTextBox_ClassName
	...
	*/
	/******************Click Text ************************/
	public void clearText(String xpathKey) {
		try {
			
			getElement(xpathKey).clear();
			reportPass(" Clear successfully in locator Element : " + xpathKey);
		} catch (Exception e) {
			reportFail(e.getMessage());
		}
		//getElement(xpathKey).clear();
	...
	public void dropDownElement(String xpathKey, String visibleText) {
		try {
			
			we = getElement(xpathKey);
			Select dp = new Select(we);
			visibleText = visibleText.split("\\.")[0];
			System.out.println(xpathKey +" : " +visibleText);
			
			//visibleText= visibleText.toUpperCase();
			//System.out.println(xpathKey +" : " +visibleText);
		
			dp.selectByVisibleText(visibleText);
			reportPass(xpathKey + visibleText +" : dropDownElement Clicked Successfully");
		} catch (Exception e) {
			reportFail(e.getMessage());
		}
	}
